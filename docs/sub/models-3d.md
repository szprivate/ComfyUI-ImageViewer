...
Navigation follows Maya: hold <kbd>Alt</kbd> and the left button tumbles, the middle button tracks and the right button dollies. Tumbling turns about **the object under the cursor** — press on a chair and the view swings around that spot on the chair, which stays put under the pointer; press on empty space and it turns about the middle of the view as before. A dolly afterwards heads for the object you tumbled around. It works the same through a scene camera, where it moves that camera. The pointer says which it is — an arrow that picks and drags, a hand while <kbd>Alt</kbd> is held. Without <kbd>Alt</kbd> the left button belongs to the scene — click an object in the viewport to select it, or a camera's frustum lines, and drag the gizmo to move it. The numbers follow, and so does the scene. It is the same on every 3D tab, whether it holds one model or fifty.
...
Any item can hold others, not only a group — drop a light's stand onto the car and it rides along. Dragging a row that is part of the selection moves the whole selection, in the order it had; every drop is one undo step, and a row put back where it was leaves none. **Nothing moves when it changes parent** — as in Maya's outliner, an item keeps its place in the world, and its translate, rotate and scale are worked out again relative to its new parent (its pivot and any frozen transform stay as they are). An animated item has each of its keys worked out again at its own frame, so its motion stays where it was too. The same goes for **Group**, when the items come from different parents. A camera has no scale of its own, so one dropped into a scaled group keeps its position and view but is drawn at the group's scale.
